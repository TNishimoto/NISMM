...
public:
  ...
  void Init(const vector<VC> &str_set) {
    ...
    for (Size idx=N_-1; idx>0; --idx) {
      ...
      for (char b=0; b<2; b++) {
        dest_[idx][b].resize(nxt_dict.size()+1);
        reset_length_[idx][b].resize(nxt_dict.size()+1);
        reported_[idx][b].resize(nxt_dict.size()+1);
      }
      ...
      for (char b=0; b<2; b++) { 
        assert(dest_[idx][b].size() > nxt_dict.size());
        assert(reset_length_[idx][b].size() > nxt_dict.size());
        Size qc = 1 + (idx-1)*2 + (Size)b;
        auto subt = cst_.child(root, qc);
        if (subt == root) {
          dest_[idx][b][nxt_dict.size()] = dict.size();
          reset_length_[idx][b][nxt_dict.size()] = 0;
        } else {
          auto nidx = ReverseDict(nxt_dict, subt);
          dest_[idx][b][nxt_dict.size()] = nidx;
          reset_length_[idx][b][nxt_dict.size()] = 1;
        }
      }
      dict.swap(nxt_dict);
    }
  ...
  bool ExportAsCircuit(const char *database_path, const char *indices_path, const char *delim_path) {
    ...
    for (auto idx=N_-1; idx > 0; idx--) {
      printf("%u transitions remaining... current size: %u breadth: %u\n", idx, num_logic_, breadth_report_[idx]);
      //if ((N_-idx)%100 == 0) printf(" %.3f%%...\n", (N_-idx)/span);
